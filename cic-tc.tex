...
(\citeauthor{chen2015compressed}, \citeyear{chen2015compressed}, tradução própria).
As formas tradicionais de aquisição de sinais baseadas nos teoremas de Shannon e Nyquist acabam por causar o discarte
...
\begin{equation}
    y_i = \langle \hat{\phi_i}, \hat{f} \rangle \text{ for } i=1,2,...,m, 
\end{equation}
na qual $m \ll d$. Os vetores \textbf{$\hat{\phi_i}$} são colunas de uma matriz $m \times n$ $\Phi$, chamada matriz de amostragem.
Assim, podemos reescrevem o vetor de amostragem como \textbf{$y = \Phi \hat{f}$}. Fica claro que, se $ m \ll d$, reconstruir
\textbf{$\hat{f}$} a partir de \textbf{$y$} é um problema com infinitas soluções, sem assumir nada a mais. \cite{chen2015compressed}.
