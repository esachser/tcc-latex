...
\section{Introdução a CUDA}

\chapter{Trabalhos Relacionados}
Neste capítulo serão abordados alguns trabalhos que se relacionam com a proposta apresentada.
O foco principal da proposta está na codificação esparsa de frames utilizando dicionários
previamente treinados, focando em vídeos e buscando fazer o processo em tempo real.
...
pixel a pixel, RGB (Red, Green, Blue). Ou seja, a entrada para essa etapa é um frame 
$m \times n \times 3$,
sendo a resolução do vídeo $m \times n$. No caso do sistema criado, a ordem dos pixels é 
linha a linha, ou seja, primeiro a linha $0$ e todas suas colunas, depois a $1$, e assim por diante.
Em contraste com essa definição poderíamos ter a imagem coluna por coluna, o que alteraria
...
\begin{enumerate}
    ...
    foram vetorizados conforme \textbf{CITAR PROCEDIMENTO DOS PATCHES}.
...
deve ser executada antes da transmissão de tempo real. Isso é possível, mesmo que não
...
A implementação básica escolhida para o sistema foi baseada na biblioteca SPAMS.
%% CIIIIIIIIIIIIITARRR
...
uma taxa diferente sem ser necessários mais cálculos nessa etapa.

Como resultado final dessa etapa, teremos, para cada patch $i$, uma matriz de representação
...
\textit{stream} de bytes, para ser enviada ou armazenada pelo pelo \textit{encoder}.
...
A partir desse parâmetro escolhem-se as colunas $q$ de cada $\mat{\Gamma_i}$ (já quantizado), 
...
Por fim, inicializada com zeros uma matriz $\mat{\Gamma} \in \mathbb{R}^{K \times T}$,
basta associar $\mat{\Gamma_{\mat{R_{:,i}}, i}} = \mat{V_{:,i}}$ para todo $1 \le i \le T$.
...
De tal modo, podemos definir que a matriz de patches $\mat{X} = \mat{D} \mat{\Gamma}$.
...
a imagem recebida $I$ de dimensões $m \times n$.
