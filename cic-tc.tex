...
compressibilidade.
A figura abaixo mostra que o PSNR é bem mais estável utilizando uma representação de 16bit.

\begin{figure}[H]
    \caption{PSNR médio em função de $K$, com quantização de 16bit.}
    \begin{center}
        \includegraphics[width=0.48\textwidth]{img/graficos/p4_ds_psnr_16bit.png}
        \includegraphics[width=0.48\textwidth]{img/graficos/p8_ds_psnr_16bit.png}
    \end{center}
    \legend{Fonte: O autor.}
    \label{fig:psnr16bit}
\end{figure}

Apesar da percepção de que alcança maior estabilidade utilizando 16bit na quantização, 
também é possível verificar que não acontece grande alteração no PSNR devido ao uso 
dos 16bit, nem mesmo devido ao aumento de $K$.
Claramente é necessário ressaltar o caso dos vídeos indoor, que apresentam bastante diferença de
PSNR na relação 8bit para 16bit.
Partindo desse fato, pode-se constatar que utilizar um dicionário menor não compromete
significativamente a qualidade do vídeo.
...
Sumarizando, foram escolhidos $p=4$, $K=32$, e $s=3,4,5,6$.

\begin{figure}[H]
    \caption{PSNR médio em função de $s$.}
    \begin{center}
...
Os pontos $s=4,5,6$ do vídeo indoor apresentaram leves baixas no PSNR. 
Isso se deve ao erro de quantização envolvido no processo.

Por fim, foi aplicada a métrica de \emph{bitrate},
que consiste em calcular quandos bits por segundo a representação compactada 
de cada vídeo conseguiu alcançar.

Foi percebido que a melhor compressão aconteceu com $p=8$ e $K=192$, para todos 
os vídeos testados. 
Além disso, é possível verificar que o PSNR médio não tem tantas mudanças em relação 
a mudança do tamanho do patch ou do dicionário.
Portando, foi calculado o bitrate usando essa configuração.

% Tabela do bitrate
\begin{table}[h]
    \caption{Bitrate calculado para vídeos de teste com $p=8$ e $K=192$.}
    \centering
        \begin{tabular}{|l|c|c|c|c|}
          ...
          \hline
          \hline
          big\_buck\_bunny & $14315$ & $24$ & $33.286.469$ & $436$ \\
          football &  $360$ & $30$ & $3.822.334$ & $2488,5$ \\
          johnny  &   $600$ & $60$ & $8.576.773$ & $6700,6$ \\
          park\_joy & $500$ & $50$ & $12.456.469$ & $9731,6$ \\
          stockholm & $604$ & $59,94$ & $12.014.763$ & $9386,5$ \\
          \hline
        \end{tabular}
    \legend{Fonte: O Autor}
    \label{tbl:bitrate}
...
de $p$, $s$ e $K$ mais baixos, enquanto para que haja melhor compressão, devem ser 
utilizados valores mais altos.
O valor de $s$ é capaz de controlar a qualidade, a compressão e até mesmo a velocidade 
...
Para a comparação com o codec H.264 foi utilizado o software FFMPEG, tendo como entrada
...
\begin{table}[h]
    ...
    \centering
        \begin{tabular}{|l|c|c|}
          ...
          \hline
          \hline
          big\_buck\_bunny & $28,83$ & $33,95$ \\
          football         & $29,62$ & $32,73$ \\
          johnny           & $35,28$ & $39,70$ \\
          park\_joy        & $26,07$ & $25,96$ \\
          stockholm        & $23,63$ & $32,80$ \\
          \hline
        \end{tabular}
    \legend{Fonte: O Autor}
