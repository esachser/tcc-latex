...
O capítulo \ref{cap:ref} apresenta o referencial teórico que serviu de base
...
As formas tradicionais de aquisição de sinais baseadas no teorema de Nyquist (\ref{teo:nyquist}) acabam por causar o descarte
de boa parte da informação coletada durante a etapa de compressão. Tal fato traz a tona a pergunta: existe 
...
que são aqueles que obedecem a seguinte lei de decaimento exponencial \cite{chen2015compressed}:
\begin{equation}
...
coeficiente seja $f_1^*$, o segundo maior, $f_2^*$, e assim por diante. $R$ é uma constante positiva e $0< q < 1$. 
Percebe-se que para valores bem pequenos de $q$ a compressibilidade se torna praticamente o mesmo que esparsidade.
% Fazer gráfico para ilustrar, aí fica mais fácil de dizer isso.
...
no máximo. Por outro lado, $\vec{f}$ pode ser esparso em alguma outra base ortonormal $\mathbf{D}$, aqui referida como 
...
\Entrada{Matriz de medidas $m \times d$ $\mathbf{\Phi}$, Vetor de $m$ medidas $\vec{y}$, parâmetro de esparsidade $s$}
...
Nesse primeira comparação pode-se verificar que altas frequências foram adicionadas a imagem,
na comparação entre a imagem de referência e a imagem gerada pelo codec, 
o que se deve a erros relacionados a quantização.
