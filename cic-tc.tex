...
$\vec{f}_s$ o vetor com os $s$ maiores valores de $\vec{f}$ em magnitude, temos que, para sinais compressíveis $\vec{f}$ e 
$\vec{f}_s$:
\begin{equation}
    \lVert \vec{f} - \vec{f}_s \rVert_2 \le Rs^{\frac{1}{2} - \frac{1}{q}} \hspace{1em} \text{ e } \hspace{1em}
    \lVert \vec{f} - \vec{f}_s \rVert_1 \le Rs^{1 - \frac{1}{q}} 
\end{equation} 
...
\begin{teorema}
    ...
    \begin{equation*}
        \lVert \vec{f}' - \vec{f} \rVert_2 \le C \left[ \epsilon + \frac{\lVert \vec{f} - \vec{f}_s \rVert_1}{\sqrt{s}} \right],
    \end{equation*}
    tal que $\vec{f}_s$ denota o vetor com os $s$ maiores coeficientes em magnitude de $\vec{f}$ e $C$ é uma
...
Segundo \citet{GilbertOMP}, o algoritmo do OMP é o seguinte:
...
$\vec{r} \leftarrow \vec{y}$\;
$t \leftarrow 1$\;
$\Lambda_0 \leftarrow ()$\;
\While{$t \le s$}{
    $\lambda_t = \underset{j=1,...,d}{argmax}|\langle\vec{r_{t-1}}, \vec{\phi_j}\rangle|$\;
    $\Lambda_t = \Lambda_{t-1} \cup \left\{\lambda_t\right\}$\;
    $\mathbf{\Phi_t} = \left[ \mathbf{\Phi_{t-1}} \hspace{0.5em} \vec{\phi_{\lambda_t}} \right]$\;
    Resolver problema de mínimos quadrados: $\vec{x_t} = \underset{\vec{x}}{argmin} \lVert \vec{y} - \mathbf{\Phi_t} \vec{x} \rVert_2$\;
}
\end{algorithm}
