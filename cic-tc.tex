...
\end{equation}
na qual $m \ll d$. Os vetores \textbf{$\hat{\phi_i}$} são colunas de uma matriz $m \times n$ $\Phi$, chamada matriz 
de amostragem.Assim, podemos reescrever o vetor de amostragem como \textbf{$\hat{y} = \Phi \hat{f}$}. Fica claro que, 
se $ m \ll d$, reconstruir\textbf{$\hat{f}$} a partir de \textbf{$\hat{y}$}, sem assumir nada a mais, é um problema 
