...
\begin{englishabstract}{An application of compressed sensing and dictionary learning techniques for real-time video coding and decoding}
    ...
    % coding methods must be developed, this ones capable of compress 
...
Um dos principais algoritmos é o \textit{Orthogonal Matching Pursuit (OMP)}, analisado por \citet{GilbertOMP}.

A ideia principal do algoritmo é, a cada iteração, escolher a coluna da matriz $\mat{\Phi}$ 
que captura mais energia do sinal residual.
O sinal residual começa sendo a amostragem de entrada.
A cada nova iteração, a partir da escolha da coluna, 
são recalculados todos os coeficientes da representação esparsa e, 
em seguida, reduzido o sinal residual, 
eliminando a contribuição já calculada.
Dessa forma, dado um parâmetro de esparsidade $s$, 
depois de $t \le s$ iterações, o OMP já calculou
a melhor representação t-esparsa da amostragem de entrada.
...
% normalmente é escolhido de forma aleatória,
...
Os códigos utilizados para a parte de aprendizado dos dicionários 
estão disponíveis em \url{https://github.com/esachser/compressive}.
Já a implementação do codec está em \url{https://github.com/esachser/PipelineTCC}.
...
bilateral, com uso de funções Gaussianas, parâmetros $\sigma_c = 20$ e $\sigma_s = 20$, 
utilizando a implementação da biblioteca OpenCV \cite{OpenCV}.
...
Os valores de PSNR médio da \autoref{sec:parametrizacao} foram calculados com base na
solução utilizando o filtro bilateral.
